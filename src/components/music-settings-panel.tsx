...
export function MusicSettingsPanel({
  ...
  const handleBassChange = useCallback((newBass: number[]) => {
    ...
    if (audioChainRef.current) {
      // Map 0-100 slider to -24 to +24 dB (more bass boost)
      const bassGain = ((newBass[0] - 50) / 50) * 24;
  ...
  const handleTrebleChange = useCallback((newTreble: number[]) => {
    ...
    if (audioChainRef.current) {
      // Map 0-100 slider to -24 to +24 dB (more treble boost)
      const trebleGain = ((newTreble[0] - 50) / 50) * 24;
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className={cn(
                ...
                {bass[0] > 50 ? "+" : ""}{Math.round(((bass[0] - 50) / 50) * 24)}
              </span>
              {!bassEnabled && (
            ...
            >
              <span className="font-mono-grit text-[10px] text-muted-foreground w-8">-24</span>
              <Slider
                value={bass}
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className={cn(
                ...
                {treble[0] > 50 ? "+" : ""}{Math.round(((treble[0] - 50) / 50) * 24)}
              </span>
              {!trebleEnabled && (
                <span className="font-mono-grit text-[9px] text-muted-foreground animate-pulse">MOVE TO ACTIVATE</span>
              )}
